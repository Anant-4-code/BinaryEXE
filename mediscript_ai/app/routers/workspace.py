...
async def _run_all_extractions_core(prescription_id: int, db: Session) -> None:
    ...
    pd = await call_gemma_extract_patient_doctor(prescription.raw_text)
    prescription.patient_details_json = json.dumps(pd.get("patient") or {})
    prescription.doctor_details_json = json.dumps(pd.get("doctor") or {})
