...
def generate_prescription_pdf(
    ...
    def draw_medicines_table(y: float) -> float:
        # Name, Strength, Dosage, Frequency, Duration, Instructions, Confidence
        # 7 columns => 8 boundaries. Keep a dedicated rightmost band for the confidence badge.
        cols = [0.0, 0.22, 0.32, 0.45, 0.58, 0.70, 0.92, 1.0]
        ...
        for idx, m in enumerate(meds, 1):
            ...
            pdf.drawString(col_x[1] + 0.08 * inch, y_cursor - 0.24 * inch, str(strength)[:18])
            pdf.drawString(col_x[2] + 0.08 * inch, y_cursor - 0.24 * inch, str(dose)[:14])
            pdf.drawString(col_x[3] + 0.08 * inch, y_cursor - 0.24 * inch, str(freq)[:16])
            pdf.drawString(col_x[4] + 0.08 * inch, y_cursor - 0.24 * inch, str(dur)[:12])

            inst_lines = wrap_lines(inst, body_font, 8, (col_x[6] - col_x[5]) - 0.16 * inch)
            pdf.setFont(body_font, 8)
            pdf.drawString(col_x[5] + 0.08 * inch, y_cursor - 0.22 * inch, (inst_lines[0] if inst_lines else "—")[:60])
            pdf.setFont(body_font, 9)

            # confidence badge
            badge_w = (col_x[7] - col_x[6]) - 0.16 * inch
            bx = col_x[6] + 0.08 * inch
