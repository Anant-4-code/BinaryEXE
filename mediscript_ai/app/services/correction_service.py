...
async def normalize_age_range(age_range: str) -> str:
    """Normalize age range to standard categories."""
    if not age_range or not age_range.strip():
        return "adult"
    
    age_lower = age_range.lower().strip()
    
    # Remove numbers from age range (like "10-24 years" → "")
    if any(char.isdigit() for char in age_lower):
        # It's a numeric range, convert to category
        try:
...
async def correct_medicines_batch(medicines: List[GemmaMedicine]) -> List[GemmaMedicine]:
    """
    Correct a batch of extracted medicines by:
    1. Verifying medicine names
    2. Completing missing durations
    3. Normalizing frequencies and age ranges
    4. Validating doses
    
    Args:
        medicines: List of extracted GemmaMedicine objects
    
    Returns:
        List of corrected GemmaMedicine objects
    """
    if not medicines:
        return []
    
    logger.info(f"Starting correction for {len(medicines)} medicines")
    
    corrected_medicines = []
    
    for med in medicines:
        try:
            # 1. Correct medicine name
            corrected_name = await correct_medicine_name(med.medicine)
            
            # 2. Validate and correct dose
            corrected_dose = await validate_and_correct_dose(med.dose)
            if not corrected_dose:
                corrected_dose = med.dose  # Keep original if validation fails
            
            # 3. Normalize frequency
            corrected_frequency = await normalize_frequency(med.frequency)
            
            # 4. Infer missing duration
            corrected_duration = med.duration
            if not corrected_duration or "—" in corrected_duration or "?" in corrected_duration:
                corrected_duration = await infer_missing_duration(
                    corrected_name,
                    corrected_dose,
                    corrected_frequency
                )
            
            # 5. Normalize age range
            corrected_age_range = await normalize_age_range(med.age_range)
            
            # Create corrected medicine
            ...
            logger.info(f"Corrected: {med.medicine} → {corrected_name}")
        
        except Exception as e:
            logger.error(f"Error correcting medicine {med.medicine}: {e}")
            # Keep original medicine if correction fails
            corrected_medicines.append(med)
    
    logger.info(f"Correction complete: {len(corrected_medicines)} medicines")
