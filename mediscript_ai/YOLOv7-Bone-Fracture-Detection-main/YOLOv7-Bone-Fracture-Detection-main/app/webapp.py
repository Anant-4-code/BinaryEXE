...
# This avoids crashing on machines without the CUDAExecutionProvider installed.
_available_providers = set(ort.get_available_providers())
device = "cuda" if "CUDAExecutionProvider" in _available_providers else "cpu"
...
if __name__ == "__main__":
    ...
    if not os.path.exists(model_onnx_path):
        st.error(f"Missing ONNX model at: {model_onnx_path}. Download yolov7-p6-bonefracture.onnx into the repo root.")
        st.stop()
